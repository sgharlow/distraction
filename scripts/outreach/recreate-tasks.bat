...
schtasks /create /tn "DistractionIndex-WeeklyCascade" /tr "cmd /c cd /d %PROJECT_DIR% && npx tsx scripts/outreach/substack-publish.ts >> scripts\outreach\cascade.log 2>&1" /sc weekly /d SUN /st 02:00 /f
...
schtasks /create /tn "DistractionIndex-EmailFollowups" /tr "cmd /c cd /d %PROJECT_DIR% && npx tsx scripts/outreach/email-campaign.ts --followup >> scripts\outreach\email-followup.log 2>&1" /sc weekly /d MON /st 09:00 /f
...
schtasks /create /tn "DistractionIndex-BlueskyEngage" /tr "cmd /c cd /d %PROJECT_DIR% && npx tsx scripts/outreach/bluesky-engage.ts >> scripts\outreach\engage.log 2>&1" /sc daily /st 10:00 /f
