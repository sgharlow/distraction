 ...
 *
 * Runs daily and checks the two failure modes the July 2026 outage exposed:
 ...
 *
 * If EITHER is unhealthy it emails the operator via Resend (Supabase-independent,
...
export async function GET(request: NextRequest) {
  ...
  if (!freshness.healthy || !stuck.healthy) {
    alert = await sendHealthAlert({ freshness, stuck });
    console.error(
  ...
  return NextResponse.json({ ok: true, freshness, stuck, alert });
}
