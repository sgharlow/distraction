  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-[900px] px-5 py-6">
        ...
        <p className="font-sans text-[9px] text-text-dim tracking-[0.5px] uppercase mb-5">Full algorithmic transparency</p>

        {/* What's new */}
        <section className="bg-action-light border border-surface-border rounded-[6px] p-3 mb-3">
          <h2 className="font-serif text-[13px] font-bold text-action mb-1">New in v2.2 — Weekly Editions</h2>
          <p className="text-[13px] text-text-secondary leading-relaxed m-0">
            The Distraction Index publishes as <strong className="text-text-primary">weekly editions</strong>. Each week
        ...
        <div className="space-y-2.5">
          ...
          <section className="bg-damage-light border border-surface-border rounded-[6px] p-3">
            ...
            <div className="font-sans text-xs text-damage bg-surface-base rounded-[3px] p-2 mb-3">
              A = min(100, 100 × Σ(weight × driver / 5) × avg(severity) × mechanism × scope)
            </div>

            <h3 className="font-sans text-[9px] font-semibold text-damage mb-1.5 uppercase tracking-[2px]">
              Drivers & Weights
            </h3>
            <div className="space-y-0.5 mb-3">
              {A_DRIVER_KEYS.map((key) => (
                <div key={key} className="flex justify-between text-[13px]">
                  ...
                  <span className="font-sans text-text-dim">{A_DRIVER_WEIGHTS[key]}</span>
                </div>
              ))}
            </div>
            ...
            <ul className="text-[13px] text-text-secondary space-y-0.5 list-none p-0 m-0 mb-3">
              <li><strong className="text-text-primary">Durability</strong> — How lasting is the change?</li>
              <li><strong className="text-text-primary">Reversibility</strong> — Can it be undone by next administration?</li>
              <li><strong className="text-text-primary">Precedent</strong> — Does it create dangerous new norms?</li>
            </ul>
            ...
            <div className="space-y-0.5">
              {(Object.keys(MECHANISM_MODIFIERS) as Array<keyof typeof MECHANISM_MODIFIERS>).map((key) => (
                <div key={key} className="flex justify-between text-[13px]">
                  ...
                  <span className="font-sans text-text-dim">×{MECHANISM_MODIFIERS[key]}</span>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="bg-distraction-light border border-surface-border rounded-[6px] p-3">
            ...
            <div className="font-sans text-xs text-distraction bg-surface-base rounded-[3px] p-2 mb-3">
              B = 100 × (0.55 × L1_avg/5 + intent_weight × L2_avg/5)
            </div>

            <h3 className="font-sans text-[9px] font-semibold text-distraction mb-1 uppercase tracking-[2px]">
              Intentionality Thresholds
            </h3>
            <div className="text-[13px] text-text-secondary space-y-0.5 mb-3">
              <div>≥ 8/15 → Full strategic weight (<span className="font-sans">0.45</span>)</div>
              <div>4–7/15 → Reduced weight (<span className="font-sans">0.25</span>)</div>
              <div>&lt; 4/15 → Minimal weight (<span className="font-sans">0.10</span>)</div>
            </div>
          </section>
          ...
          {/* Smokescreen */}
          <section className="bg-mixed-light border border-surface-border rounded-[6px] p-3">
            <h2 className="font-serif text-sm font-bold text-mixed mb-1">Smokescreen Index</h2>
            ...
            <div className="font-sans text-xs text-mixed bg-surface-base rounded-[3px] p-2">
          ...
          {/* Freeze policy */}
          <section className="bg-surface-overlay border border-surface-border rounded-[6px] p-3">
            <h2 className="font-serif text-sm font-bold text-text-dim mb-1">Weekly Freeze Policy</h2>
            <p className="text-[13px] text-text-secondary leading-relaxed">
