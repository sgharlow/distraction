    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-[900px] mx-auto px-4 py-2">
        <div className="flex gap-3 flex-wrap">
          {Array.from({ length: 5 }).map((_, i) => (
            <div
              key={i}
              className="h-10 w-24 bg-surface-raised border border-surface-border rounded-[6px] animate-pulse"
            />
          ))}
        </div>
      </div>
      ...
      <div className="max-w-[900px] mx-auto px-4 py-2">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 mb-4">
          <div className="bg-damage/[0.03] border border-damage/[0.08] rounded-[6px] p-3 h-24 animate-pulse" />
          <div className="bg-distraction/[0.03] border border-distraction/[0.08] rounded-[6px] p-3 h-24 animate-pulse" />
        </div>
      </div>
          ...
          {(['damage', 'distraction', 'noise'] as const).map((color) => (
            <div key={color} className="space-y-2">
              ...
              {Array.from({ length: 4 }).map((_, i) => (
                <div
                  key={i}
                  className={`bg-${color}/[0.02] border border-${color}/[0.06] rounded-[6px] p-3`}
                >
